...
ErrorCode I2C_Peripheral_ReadRegister(uint8_t EZI2C_address, 
                                      uint8_t* buffer_address,
                                      uint8_t data) {
    
    // Start Condition
    uint8_t error = EZI2C_curState;                                        
    if (error != (EZI2C_STATUS_ERR))
    {
        if (error != (EZI2C_STATUS_BUSY))
        {
           
        
        //Write the register address to be read
            EZI2C_SetAddress1(EZI2C_address);
            EZI2C_SetBuffer1(2,2,buffer_address);
            {    
            //NO ERROR IN THE BRIDGE CONTROL PANEL IS REPRESENTED AS A +
            // Send a restart condition
           
            {
                *data = EZI2C_Re(I2C_Master_NAK_DATA); //These last two functions are already declared in my API
            }
        }
    }
    // Stop Comunication
